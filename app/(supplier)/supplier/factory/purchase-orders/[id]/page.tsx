...
export default function FactoryOrderDetailPage({ params }: { params: { id: string } }) {
    ...
    if (error || !order) {
        return (
            <div className="space-y-4">
                ...
                <Link href="/factory/purchase-orders" className="text-sm text-blue-400 hover:text-blue-300">
                    ← Back to Orders
                </Link>
            </div>
        );
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-6">
                        ...
                        {order.deliveryAddress && (
                            <div className="mt-4 pt-4 border-t border-slate-800">
                                <dt className="text-sm text-slate-400 mb-1">Delivery Address</dt>
                                <dd className="text-sm text-slate-100">{order.deliveryAddress}</dd>
                            </div>
                        )}
                    </div>
                    ...
                    {nextStatus && (
                        <div className="rounded-xl border border-blue-500/30 bg-blue-500/10 p-6">
                            <h3 className="font-medium text-blue-400 mb-4">Update Status</h3>
                            <p className="text-sm text-slate-300 mb-4">
                                Mark this order as <strong>{nextStatus.replace(/_/g, " ")}</strong>
                            </p>
                            <button
                                onClick={() => handleUpdateStatus(nextStatus)}
                                disabled={updating}
                                className="rounded-lg bg-blue-500 px-6 py-3 text-sm font-medium text-slate-950 hover:bg-blue-400 disabled:opacity-50"
                            >
                    ...
                    <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-6">
                        ...
                        <p className="text-sm text-slate-400 mb-4">Contact the operations team with any questions.</p>
                        <Link
                            href="/factory/messages"
