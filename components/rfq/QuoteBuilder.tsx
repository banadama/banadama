// components/rfq/QuoteBuilder.tsx - RFQ Quote Generation Component
"use client";
...
export function QuoteBuilder({ rfqId }: { rfqId: string }) {
    ...
    async function submitQuote() {
        ...
        await run(
            () => apiPost(`/api/rfq/${rfqId}/quote`, {
                itemsTotal: parseFloat(payload.itemsTotal) || 0,
                shipping: parseFloat(payload.shipping) || 0,
    ...
    return (
        <div className="bd-card bd-card-pad" style={{ boxShadow: "none" }}>
            <div style={{ display: "grid", gap: 10 }}>
                <div style={{ fontWeight: 900 }}>Generate Quote</div>

                <input
                    className="bd-input"
                    type="number"
                    placeholder="Items total (₦)"
                    value={payload.itemsTotal}
                    onChange={(e) => setPayload({ ...payload, itemsTotal: e.target.value })}
                />
                <input
                    className="bd-input"
                    type="number"
                    ...
                    onChange={(e) => setPayload({ ...payload, shipping: e.target.value })}
                />
                <input
                    className="bd-input"
                    type="number"
                    placeholder="Fees (₦)"
                    value={payload.fees}
                    onChange={(e) => setPayload({ ...payload, fees: e.target.value })}
                />
                <textarea
                    className="bd-input"
