// components/admin/actions/control/MarketControlEditor.tsx
"use client";
...
export function MarketControlEditor({ config }: { config: any }) {
    ...
    async function save() {
        await run(
            () => apiPatch(`/api/admin/market-control`, state),
            { start: "Updating market control", ok: "Market control saved", fail: "Update failed" }
        );
    }
    ...
    return (
        <div className="bd-card bd-card-pad" style={{ boxShadow: "none" }}>
            <div style={{ display: "grid", gap: 10 }}>
                <div style={{ fontWeight: 950 }}>Market Control</div>
                <textarea
                    className="bd-input"
                ...
                <button className="bd-btn bd-btn-primary" onClick={save} disabled={loading || error} style={{ justifyContent: "center" }}>
                    <Icons.Product size={18} /> {loading ? "Saving..." : "Save"}
                </button>
            </div>
        </div>
    );
